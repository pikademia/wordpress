...
function pgc_sgb_amp_item( $item )
{
    ...
    if ( $item['type'] === 'image' || $item['type'] === 'audio' ) {
        $itemElemant = '<img alt="' . esc_attr( ( isset( $item['alt'] ) ? $item['alt'] : '' ) ) . '" width="' . esc_attr( $image['width'] ) . '" height="' . esc_attr( $image['height'] ) . '" data-lazy-src="" class="skip-lazy no-lazyload noLazy" ' . 'src="' . esc_url( $image['src'] ) . '"/>';
...
function pgc_sgb_render_callback( $atr, $content )
{
    ...
    $preloder = '<div class="sgb-preloader" id="pr_' . $atr['galleryId'] . '" style="display: none;">
    ...
    $html = '<div class="pgc-sgb-cb ' . $className . '" data-gallery-id="' . $atr['galleryId'] . '"' . (( isset( $style ) ? $style : '' )) . '>' . $preloder . '<script type="application/json" class="sgb-data">' . $galleryData . '</script>' . '<script>(function(){var sgbPrel = document.getElementById("pr_' . $atr['galleryId'] . '"); if(sgbPrel){sgbPrel.style.display="block";} if(window.PGC_SGB && window.PGC_SGB.searcher){window.PGC_SGB.searcher.initBlocks()}})()</script>' . $noscript . '</div>';
    return $html;
}
